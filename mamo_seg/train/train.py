...
import numpy as np
from torchmetrics import JaccardIndex, Dice, Accuracy
...
class MamoSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        self.num_classes = get_parameter(config, ["LightningModule", "model_params" ,"num_classes"])
        ...
        self.model = get_segmentation_model(self.model_name, self.model_params) 
        ...
        num_classes = get_parameter(config, ["LightningModule", "model_params", "num_classes"])
        
        self.val_iou = JaccardIndex(num_classes=num_classes, task="multiclass", average="none")
        #self.val_dice = Dice(num_classes=num_classes, average="none")
        self.val_accuracy = Accuracy(num_classes=num_classes, task="multiclass", average="none")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_accuracy.update(preds, mask)
    ...
    def on_validation_epoch_end(self):
        ...
        # dice_per_class = self.val_dice.compute()
        # print("Per-class Dice:", dice_per_class)
        acc_per_class = self.val_accuracy.compute()
        print("Per-class Accuracy:",acc_per_class)
        
        self.log("val/iou", iou_per_class[1:].mean(), prog_bar=False)
        #self.log("val/dice", dice_per_class[1:].mean(), prog_bar=False)
        self.log("val/accuracy", acc_per_class[1:].mean(),prog_bar=False)
...
def get_logger(config):
    ...
    print("Logger type: ", get_parameter(config, ["Logger", "type"]))
...
def create_callbacks(config):
    ...
    callbacks_dict = get_parameter(config, ["Callbacks"], default={})
...
def str_to_bool(s):
    if s.lower() == 'true':
        ...
        raise ValueError("Cannot convert string to bool. Expected 'true' or 'false'.")
...
if __name__ == "__main__":
    ...
    model = MamoSegmentation(config=config)
    data_module = CBISSegmentationDataModule(config=config)
